...
- 가중치(w)와 은닉층 뉴런(a) 표기법
  ...
  - 기호 오른쪽 위에 윗첨자 - 가중치가 가리키는 층
  - 기호 오른쪽 아래 아랫첨자 두개 - 차례로 다음 층 뉴런의 인덱스와 앞 층 뉴런의 인덱스 번호
...
- 소프트맥스 함수 개선
  ...
  :point_right: 소프트맥스의 지수 함수를 계산할 때, 어떠한 정수(C')를 더하든 빼든 **결과가 바뀌지 않음**

  - 오버플로우를 막을 목적이므로 입력 신호 중 최대값을 이용하는 것이 일반적임
...
- 출력 총합이 **1**이고, 출력은 0과 1사이의 실수 :point_right: 소프트맥스 함수의 출력을 **'확률'**관점으로 해석 가능(비중함수)
